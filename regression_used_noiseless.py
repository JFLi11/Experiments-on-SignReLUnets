...
def load_data(num_data, dim, batch_size):
    ...
    train_label = func(DATASET_train, False)
...
def exp(AC):
    ...
    np.save('./results_noiseless/%s.npy'%(AC),test_losses)
...
def plot_n_save(AC, j, jj):
    ...
    data = np.load('./results_noiseless/%s.npy'%(AC))
    
    mean = np.mean(data, axis=0)
    var = np.var(data, axis=0)
    
    
    # mean and var for excel
    excel = np.concatenate((mean, var),axis=0)
    
    
    columns = samples
    data = pd.DataFrame(excel, columns=columns)
    PATH = './results_noiseless/%s.xlsx'%(AC)
    ...
    for i in range(len(dim)):
        ...
        plt.savefig('./images_noiseless/%s%d.png'%(AC, dim[i]), dpi=400, bbox_inches='tight')
